...
function getPrayerTimes(lat, lon) {
  fetch(
    ...
    .then((data) => {
      ...
      detectNextPrayer(timings);
    })
    .catch(() => {
      document.querySelector("section:nth-child(2)").innerHTML =
        "اختار المدينة او اعطي سماحية للموقع بايجاد موقعك لعرض مواقيت الصلاة";
    });
}
...
function updatePrayerTimes(timings) {
  prayerIds.forEach((id) => {
    prayers[id].textContent = formatTime(
      timings[id.charAt(0).toUpperCase() + id.slice(1)],
    );
  });
}

//! Detect Next Prayer
function detectNextPrayer(timings) {
  let nowMinutes = today.getHours() * 60 + today.getMinutes();

  prayerIds.forEach((id, index) => {
    let prayerMinutes = timeToMinutes(
      timings[id.charAt(0).toUpperCase() + id.slice(1)],
    );

    if (nowMinutes < prayerMinutes) {
      document.getElementById("next").textContent = prayerNames[index];
      document
        .querySelector(`.prayer-time:nth-child(${index + 1})`)
...
function renderGovernorates() {
  governorates.forEach((gov) => {
    menu.innerHTML += `
    <li class="p-2 hover:bg-gray-200 cursor-pointer">${gov}</li>
    `;
  });
}
